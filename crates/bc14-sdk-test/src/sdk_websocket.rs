...
use crate::sdk_compression::decompress_server_message;
use crate::sdk_metrics::CLIENT_METRICS;
...
impl WsConnection {
    ...
    pub fn parse_response(bytes: &[u8]) -> Result<ServerMessage<BsatnFormat>, WsError> {
        let bytes = &*decompress_server_message(bytes)?;
        bsatn::from_slice(bytes).map_err(|source| WsError::DeserializeMessage { source })
    }

    pub fn encode_message(msg: ClientMessage<Bytes>) -> WebSocketMessage {
